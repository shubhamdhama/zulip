...
class JSONFetchAPIKeyTest(ZulipTestCase):
    ...
    def test_admin_only_api_key_access(self) -> None:
        realm = get_realm('zulip')
        realm.add_bot_by_user_permissions = Realm.ADMINS_ONLY
        realm.save()
        self.realm = realm
        self.login(self.email)
        result = self.client_post("/json/fetch_api_key",
                                  dict(user_profile=self.user_profile,
                                       password=initial_password(self.email)))
        self.assert_json_error(result, "Must be a realm adminstrator", 400)
